...
def create_layer_zip() -> str:
    ...
    shell(f'bash {cloud_dir}/scripts/aws-lambda-pip.sh'
          ...
          f'  -req "{this_dir}/cfnres/str_random/requirements.txt"')
...
def create_lambda_zip() -> str:
    ...
    with zipfile.ZipFile(lambda_zip, 'w', zipfile.ZIP_DEFLATED) as zip:
        zip.write(f'{this_dir}/cfnres/str_random/index.py', arcname='index.py')

    return lambda_zip


def create_lambda(stack: core.Stack) -> aws_lambda.CfnFunction:
    lambda_scope = core.Construct(stack, 'StrRandomLambda')
...
def CfnStrRandom(stack: core.Stack, scope: core.Construct, id: str, length: int) -> aws_cloudformation.CfnCustomResource:
    lambda_function: aws_lambda.CfnFunction = cdk.find_resource(stack, 'StrRandomLambda/Function')
    if not lambda_function: lambda_function = create_lambda(stack)

    custom_resource = aws_cloudformation.CfnCustomResource(
        scope, id,
        service_token=lambda_function.attr_arn
    )
    custom_resource.add_override('Type', 'Custom::StrRandom')
    custom_resource.add_override('Properties.Length', length)
