...
def create_layer_zip() -> str:
    ...
    shell(f'bash {cloud_dir}/scripts/aws-lambda-pip.sh'
          ...
          f'  -req "{this_dir}/cfnres/iot_cacert/requirements.txt"')
...
def create_lambda_zip() -> str:
    ...
    with zipfile.ZipFile(lambda_zip, 'w', zipfile.ZIP_DEFLATED) as zip:
        zip.write(f'{this_dir}/cfnres/iot_cacert/index.py', arcname='index.py')

    return lambda_zip


def create_lambda(stack: core.Stack, iot_scope: core.Construct) -> aws_lambda.CfnFunction:
    lambda_scope = core.Construct(iot_scope, 'CaCertificateLambda')
    ...
    lamba_asset = stack.synthesizer.add_file_asset(
    ...
    lambda_function = aws_lambda.CfnFunction(
        ...
        code=aws_lambda.CfnFunction.CodeProperty(
            s3_bucket=lamba_asset.bucket_name,
            s3_key=lamba_asset.object_key
        ),
...
def create(stack: core.Stack) -> None:
    ...
    custom_resource = aws_cloudformation.CfnCustomResource(
        iot_scope, 'CaCertificate',
        service_token=lambda_function.attr_arn
    )
    custom_resource.add_override('Type', 'Custom::IotCaCertificate')
    custom_resource.add_override('Properties.CertificateProperties', stack.node.try_get_context('cacert'))
    custom_resource.add_override('Properties.ArnStore', f'/{cdk.app_name}/cacert')
